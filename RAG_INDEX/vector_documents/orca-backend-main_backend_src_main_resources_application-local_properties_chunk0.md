# Knowledge Document: application-local.properties (Chunk 1/1)

## Metadata
```json
{
  "file_path": "orca-backend-main/backend/src/main/resources/application-local.properties",
  "language": "properties",
  "module": "resources",
  "business_domain": "dashboard",
  "tags": [
    "dashboard",
    "payment"
  ],
  "logical_type": "Generic",
  "chunk_index": 0,
  "total_chunks": 1
}
```

## Semantic Context
- **Purpose**: Implementation chunk of Generic in resources.
- **Dependencies**: Refer to module imports.
- **Tags**: dashboard, payment

## Source Code Chunk
```properties
server.port=8080

spring.datasource.url=${DB_URL:jdbc:postgresql://127.0.0.1:5432/orca_db}
spring.datasource.username=${DB_USERNAME:postgres}
spring.datasource.password=${DB_PASSWORD:123456}
spring.datasource.driver-class-name=org.postgresql.Driver

spring.jpa.hibernate.ddl-auto=update
spring.jpa.show-sql=true
spring.jpa.properties.hibernate.dialect=org.hibernate.dialect.PostgreSQLDialect
spring.jpa.properties.hibernate.hbm2ddl.halt_on_error=false
spring.jpa.open-in-view=false

spring.h2.console.enabled=false

app.frontend.url=http://localhost:5173
vnpay.pay-url=${VNPAY_PAY_URL:https://sandbox.vnpayment.vn/paymentv2/vpcpay.html}
vnpay.tmn-code=${VNPAY_TMN_CODE:DEMOV210}
vnpay.hash-secret=${VNPAY_HASH_SECRET:}
vnpay.return-url=${VNPAY_RETURN_URL:http://localhost:8080/api/payments/vnpay/return}

ai.service.api-key=${AI_SERVICE_API_KEY:your_api_key_here}
ai.v2.service-url=${AI_V2_SERVICE_URL:http://127.0.0.1:8000}

# ===== PayOS Config =====
# Thay thế "YOUR_CLIENT_ID", "YOUR_API_KEY", "YOUR_CHECKSUM_KEY" bằng thông tin thật từ dashboard PayOS
payos.client-id=${PAYOS_CLIENT_ID:YOUR_CLIENT_ID}
payos.api-key=${PAYOS_API_KEY:YOUR_API_KEY}
payos.checksum-key=${PAYOS_CHECKSUM_KEY:YOUR_CHECKSUM_KEY}

```
