# Knowledge Document: _generate_schema.py (Chunk 18/58)

## Metadata
```json
{
  "file_path": "orca-backend-main/ai-service/.venv/Lib/site-packages/pydantic/_internal/_generate_schema.py",
  "language": "py",
  "module": "_internal",
...
## Semantic Context
- **Purpose**: Implementation chunk of Generic in _internal.
- **Dependencies**: Refer to module imports.
- **Tags**: factory, report

## Source Code Chunk
```py
g V1 models and V2 models (or constructs, like `TypeAdapter`) is not supported. Please upgrade `{obj.__name__}` to V2.',
                    UserWarning,
                )
            else:
                warn(
