...
function SalaryPanel({ teamId }: { teamId: string }) {
    ...
    return (
        <div style={{ background: '#fff', borderRadius: 20, padding: 0, marginTop: 24, border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.04)' }}>
            {/* Header */}
            <div style={{ 
                background: 'linear-gradient(135deg, #c4945d 0%, #a6723a 100%)',
                padding: '24px 28px',
                display: 'flex',
                alignItems: 'center',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                    ...
                    <div>
                        ...
                        <p style={{ margin: '4px 0 0', fontSize: 13, color: 'rgba(255, 255, 255, 0.9)' }}>Theo dõi & quản lý chi phí nhân sự</p>
                    </div>
                </div>
                <button onClick={() => { setShowSalary(p => !p); if (!showSalary) loadSalary(); }} style={{
                    background: showSalary ? 'rgba(0, 0, 0, 0.15)' : 'rgba(0, 0, 0, 0.25)',
                    color: '#fff',
                    border: 'none',
                    borderRadius: 12, padding: '10px 20px', fontSize: 13, fontWeight: 700, cursor: 'pointer',
                    display: 'flex', alignItems: 'center', gap: 8,
                    transition: 'all 0.2s',
                    boxShadow: showSalary ? 'none' : '0 4px 12px rgba(0, 0, 0, 0.1)'
                }}>
            ...
            {/* Month Selector */}
            <div style={{ padding: '16px 28px', borderBottom: '1px solid rgba(0,0,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#a6723a' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <button 
                        ...
                        style={{ width: 32, height: 32, borderRadius: 8, border: 'none', background: 'rgba(0,0,0,0.2)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                    >
                        <svg width="16" height="16" fill="none" stroke="#fff" strokeWidth="2"><path d="M15 18l-6-6 6-6"/></svg>
                    </button>
                    <select 
                        value={selectedMonth} 
                        ...
                        style={{ 
                            ...
                            background: 'rgba(0,0,0,0.2)', fontSize: 14, fontWeight: 600, color: '#fff',
                    ...
                    <button 
                        ...
                        style={{ width: 32, height: 32, borderRadius: 8, border: 'none', background: 'rgba(0,0,0,0.2)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                    >
                        <svg width="16" height="16" fill="none" stroke="#fff" strokeWidth="2"><path d="M9 18l6-6-6-6"/></svg>
                    </button>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            {showSalary && (
                <div style={{ background: '#5d4037' }}>
                    ...
                    <div style={{ padding: '20px 28px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
                        {[
                            { label: 'Tổng nhân viên', value: salaryData.length, icon: '👥', color: '#fff' },
                            { label: 'Tổng công việc', value: totalTasks.toLocaleString(), icon: '📋', color: '#fff' },
                            { label: 'Hoàn thành', value: `${avgCompletion}%`, icon: '✅', color: '#fff' },
                            ...
                            <div key={i} style={{ 
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                                    ...
                                    <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.7)', fontWeight: 600 }}>{stat.label}</span>
                                </div>
                    ...
                    {loadingSalary ? (
                        ...
                        <div style={{ padding: '0 28px 24px' }}>
                            ...
                            <div style={{ 
                                ...
                                background: 'rgba(0,0,0,0.3)', borderRadius: '12px 12px 0 0',
                            ...
                            {salaryData.map((s, idx) => {
                                ...
                                return (
                                    <div key={s.memberId} style={{
                                        ...
                                        borderBottom: idx < salaryData.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none',
                                        alignItems: 'center',
                                    ...
                                    >
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                            ...
                                            <div>
                                                <div style={{ fontSize: 14, fontWeight: 700, color: '#fff' }}>{s.memberName}</div>
                                                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>ID: {s.memberId?.slice(0, 8)}...</div>
                                            </div>
                                        </div>

                                        {/* Total Tasks */}
                                        <div style={{ textAlign: 'center' }}>
                                            <div style={{ fontSize: 16, fontWeight: 800, color: '#fff' }}>{s.totalTasks}</div>
                                            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.5)' }}>task</div>
                                        </div>
                                        ...
                                        <div style={{ textAlign: 'center' }}>
                                            <div style={{ 
                                                ...
                                                background: completionRate >= 80 ? 'rgba(22, 163, 74, 0.2)' : completionRate >= 50 ? 'rgba(217, 119, 6, 0.2)' : 'rgba(220, 38, 38, 0.2)',
                                            ...
                                            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.5)', marginTop: 2 }}>{completionRate}%</div>
                                        </div>

                                        {/* Workload */}
                                        <div style={{ textAlign: 'center' }}>
                                            <div style={{ fontSize: 16, fontWeight: 700, color: '#fff' }}>{s.totalWorkload.toFixed(1)}</div>
                                            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.5)' }}>giờ</div>
                                        </div>
                                            ...
                                            {isEditing ? (
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 4, justifyContent: 'center' }}>
                                                    <input 
                                                        ...
                                                        autoFocus
                                                        style={{ width: 60, padding: '4px 8px', borderRadius: 6, border: '1px solid #d4a574', fontSize: 13, textAlign: 'center', background: 'rgba(0,0,0,0.3)', color: '#fff' }}
                                                    />
                                                ...
                                                <div 
                                                    ...
                                                    style={{ 
                                                        ...
                                                        background: hourlyRateOverride[s.memberId] ? 'rgba(212,165,116,0.2)' : 'rgba(0,0,0,0.3)',
                            ...
                            <div style={{
                                ...
                                <div style={{ textAlign: 'center', color: 'rgba(255,255,255,0.5)', fontSize: 12 }}>—</div>
                                <div style={{ textAlign: 'center' }}>
                                    <div style={{
                    ...
                    {/* Action Buttons */}
                    <div style={{ padding: '16px 28px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'flex-end', gap: 12, background: 'rgba(0,0,0,0.15)' }}>
                        <button style={{ padding: '10px 20px', borderRadius: 10, border: 'none', background: 'rgba(255,255,255,0.1)', color: '#fff', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
