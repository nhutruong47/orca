...
content = content.replace(/type Severity = 'Critical' \| 'High' \| 'Medium' \| 'Low';/g, '');
...
// 6. Carefully remove the alerts section (Danh sách cảnh báo hệ thống) inside the 'overview' block!
// The previous simple replace failed because of formatting variations.
// Let's use a more robust regex to remove the <section className="admin-card">...<h3>Danh sách cảnh báo hệ thống</h3>...</section>
content = content.replace(/<section className="admin-card">\s*<div className="admin-card-head">\s*<h3>Danh sách cảnh báo hệ thống<\/h3>[\s\S]*?<\/section>/, '');

// 7. Unused imports (DO NOT delete AlertTriangle)
